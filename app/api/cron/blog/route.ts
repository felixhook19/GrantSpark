...
// require the Pro plan — until then this route can be triggered manually:
...
export async function GET(request: NextRequest) {
  ...
  if (!topic) {
    return NextResponse.json({
      ok: true,
      skipped: true,
      reason: 'All rotation topics published — extend the TOPICS list.',
    })
  }
...
Return ONLY a JSON object with keys: title, excerpt, meta_description, content_html, read_minutes. No markdown fences, no prose around it.`
  ...
  try {
    ...
    console.error('Blog cron parse error:', err)
